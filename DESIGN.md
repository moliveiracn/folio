...
├── internal/
│   ├── cli/
│   │   ├── init.go                 # folio init: create data dirs, DB, config, password
...
│   │   ├── citations.go            # /api/citations CRUD + formatted copy
...
│   │
│   ├── plugin/
│   │   ├── loader.go               # discovers and loads .lua files at startup
│   │   ├── lua.go                  # gopher-lua VM setup, folio API registration
│   │   └── citation.go             # calls registered Lua citation formatters
│   │
│   ├── repo/
│   │   ├── repo.go                 # DB interface
│   │   ├── sqlite.go               # modernc sqlite implementation
│   │   └── migrations/
│   │       ├── 001_init.sql        # docs, sessions, positions
│   │       ├── 002_annotations.sql # marks, bookmarks, highlights
│   │       └── 003_citations.sql   # citations + doc_meta extension
│   │
│   ├── service/
...
│   │   │   ├── citations.js        # citation list overlay + copy button
...
│       └── viewer.html
│
├── plugins/                        # shipped example plugins (baked into image)
│   ├── abnt.lua                    # ABNT NBR 6023 citation formatter
│   └── apa.lua                     # APA 7th edition (community example)
│
├── data/                           # .gitignored — this entire dir is a Docker volume
│   │                               # folio init creates this structure;
│   │                               # container updates never touch it
│   ├── books/                      # uploaded PDFs and EPUBs (content-addressed by SHA256)
│   ├── plugins/                    # user-installed Lua plugins (loaded alongside /plugins)
│   ├── folio.db                    # single SQLite file — backup = cp folio.db folio.db.bak
│   └── config.yaml                 # generated by folio init, contains bcrypt hash
│
├── docs/
│   ├── schema.sql                  # canonical schema (mirrors migrations, for reference)
│   └── KEYBINDINGS.md
│
├── .air.toml                       # air hot-reload config (dev only)
├── .gitignore                      # includes: data/, tmp/, *.db
├── Dockerfile
├── docker-compose.yml
├── go.mod
├── go.sum
...
### What `folio init` creates

`folio init` is the only command that writes to the filesystem outside the binary itself. It creates exactly the four things that must persist across container updates — everything else is baked into the image:

```
{data_dir}/
├── books/        ← uploaded documents, never recreated by the server
├── plugins/      ← user Lua plugins, loaded at startup alongside built-ins
├── folio.db      ← SQLite database, runs migrations on first init
└── config.yaml   ← bcrypt password hash + server config
```

The `plugins/` directory inside the repo (shipped example plugins) is embedded into the binary at build time via `//go:embed`. The `{data_dir}/plugins/` directory is where users drop their own `.lua` files. The server loads both sources at startup, with user plugins taking precedence if names collide.
...
3. Generates a 12-character random password (if `--password` not provided)
4. Hashes it with bcrypt and writes to `config.yaml`
